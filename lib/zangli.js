...
var startDate = new Date(1951, 0, 8);
var endDate = new Date(2051, 1, 11);
/*方法说明
 *@method getZangli
...
function getZangli(p) {
	...
	// Fix: Strip time from d before subtracting to avoid Math.round causing +1 day offset at noon
	var dNormalized = new Date(d.getFullYear(), d.getMonth(), d.getDate());
	var days = Math.round((dNormalized - startDate) / 86400 / 1000);
	var countingDays = 0;
	var countingMonth = 0;
	for (var years = 0; years < specialDays.length; years++) {
		...
		for (var months = 0; months < specialDays[years].length; months++) {
			...
			if (countingDays + tDays <= days) { //还没到当前月，直接累加日子
				...
				result.numericDay = tDays + 1;
				result.numericMonth = months - leapMonths + 1;
