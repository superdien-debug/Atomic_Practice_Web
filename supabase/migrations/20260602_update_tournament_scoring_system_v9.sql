-- Migration: Upgrade Tournament Scoring System v9 - Restore Ascent/Descent (+15/-15) with Bardo (24) transition exception
-- Date: 2026-06-02
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(
            (
                SELECT SUM(
                    -- 1. Base Ascent (+15) / Descent (-15) points with Bardo exception
                    (CASE 
                        WHEN h.from_realm_id = 24 THEN
                            CASE 
                                WHEN h.to_realm_id IN (15, 17, 18, 19, 20) OR h.to_realm_id >= 25 THEN 15
                                ELSE -15
                            END
                        ...
                        ELSE 0 
                    END) +
                    -- 2. Đọa xứ (1-13) & Ấn Độ giáo, Bôn giáo (22, 23) -> trừ 10 điểm
                    (CASE 
                        WHEN (h.to_realm_id >= 1 AND h.to_realm_id <= 13) OR h.to_realm_id IN (22, 23) THEN -10 
                        ELSE 0 
                    END) +
                    -- 3. 4 cõi người (17, 18, 19, 20) -> cộng 10 điểm
                    (CASE 
                        WHEN h.to_realm_id IN (17, 18, 19, 20) THEN 10 
                        ELSE 0 
                    END) +
                    -- 4. Tịnh Độ (97-103) & Cõi Phật tinh tấn (38,39,40,47,48,25,33,42,52,54,59,60,71,77,93,104) -> cộng 15 điểm
                    (CASE 
        ...
        (
            ...
            COALESCE(
                (
                    SELECT SUM(
                        -- Base Ascent/Descent points with Bardo exception
                        (CASE 
                            WHEN h.from_realm_id = 24 THEN
                                CASE 
                                    WHEN h.to_realm_id IN (15, 17, 18, 19, 20) OR h.to_realm_id >= 25 THEN 15
                            ...
                            ELSE 0 
                        END) +
                        -- Đọa xứ (1-13) & Ấn Độ giáo, Bôn giáo (22, 23) -> trừ 10 điểm
                        (CASE 
                            WHEN (h.to_realm_id >= 1 AND h.to_realm_id <= 13) OR h.to_realm_id IN (22, 23) THEN -10 
                            ELSE 0 
                        END) +
                        -- 4 cõi người (17, 18, 19, 20) -> cộng 10 điểm
                        (CASE 
                            WHEN h.to_realm_id IN (17, 18, 19, 20) THEN 10 
                            ELSE 0 
                        END) +
                        -- Tịnh Độ (97-103) & Cõi Phật tinh tấn (38,39,40,47,48,25,33,42,52,54,59,60,71,77,93,104) -> cộng 15 điểm
                        (CASE 
                            WHEN (h.to_realm_id >= 97 AND h.to_realm_id <= 103) OR h.to_realm_id IN (38, 39, 40, 47, 48, 25, 33, 42, 52, 54, 59, 60, 71, 77, 93, 104) THEN 15 
                            ELSE 0 
                        END)
